      ...
      meter(p.levelL, p.levelR)     // initialize meter with processor's level trackers
{
    ...
    tempoSyncLight.setColour(juce::Colours::limegreen);
    tempoSyncLight.setSize(30, 30);        // overall component size (glow must fit inside)
    tempoSyncLight.setCenterScale(0.70f);  // increase the inner (green) disk to 70% of diameter
    tempoSyncLight.setColour(juce::Colours::limegreen);
    delayGroup.addAndMakeVisible(tempoSyncLight);
